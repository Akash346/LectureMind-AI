...
export function VideoPanel({
  ...
  const shouldShowError =
    Boolean(errorType && errorMessage) && (status === "FAILED" || status === "READY");
  const displayMessage = loginBlockedByYouTube
    ? "YouTube blocked transcript extraction for this video. The platform requires signed-in user verification in this environment."
    : errorMessage;
    ...
    if (status === "FAILED") {
      return (
        <div className="flex aspect-video flex-col items-center justify-center rounded-xl border border-red-200 bg-red-50 px-6 text-center text-sm text-red-950 dark:border-red-900 dark:bg-red-950 dark:text-red-50">
          ...
          {displayMessage ? <p className="mt-2 opacity-80">{displayMessage}</p> : null}
          {errorType ? (
    ...
    <div className="space-y-3">
      ...
      {shouldShowError ? (
        <div className="rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-950 dark:border-amber-800 dark:bg-amber-950/40 dark:text-amber-100">
          ...
          {displayMessage ? <p className="mt-1 opacity-90">{displayMessage}</p> : null}
